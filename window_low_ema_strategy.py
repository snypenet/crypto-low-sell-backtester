...
def window_low_ema_strategy(
    ...
    print("Running Window Low EMA Strategy")
    """
    N-day-low-EMA buy/sell strategy. Buys when today's low is below the EMA of the previous low_window days' lows.
    ...
    while current_date <= last_date:
        ...
        # Calculate EMA of 'low' for the window
        window_ema = window_rows['low'].ewm(span=low_window, adjust=False).mean().iloc[-1]

        # --- 2. today's trades ---
        day_rows = df[(df['date'] == current_date) & (df.index >= next_row_idx)]
        if day_rows.empty:
            prev_window_ema = window_ema
            current_date += timedelta(days=1)
            continue
        ...
        # --- 3. BUY logic ---
        if position == 0.0 and prev_window_ema is not None and day_low < prev_window_ema:
